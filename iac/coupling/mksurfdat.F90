...
!-----------------------------------------------------------------------
    ...
    if (domain_set(ldomain) .eqv. .false.) then
    ...
    ! pctglcmec_gic and pctglcmec_icesheet appear to be leaks in the original code
    if ( nglcec > 0 ) deallocate ( pctglcmec, topoglcmec, pctglcmec_gic, pctglcmec_icesheet)
    ...
    if (mksrf_fdynuse /= ' ') then
       ...
       write(6,*) subname,'fdyndat exists value ',exists
        ...
        write(6,*) subname,'fdyndat is ', fdyndat

       ! need to create it only the first time
       ...
       ! do not read in data, and do not loop over years; it comes from plodata each year
       ! replace 'string' in the loop below with an empty string (because override is not used)
       ! replace fname below with 'plodata' when there is no override
       ...
       !do 
          ...
          do k = 1, mkharvest_numtypes()
             ...
             len(ndims) = 1
             ! ntim is already set for pfts; use the same year here
             ! this means that harvest for year is stored in year+1
             ! current and legacy E3SM standalone surface files are stored this way also
             ! the ELM code now takes care of this shift (it didn't used to)
             ! should probably fix this in version 3 to store harvest in correct year
             beg(ndims) = ntim
       ...
       call check_ret(nf_close(ncid), subname)
       ! this is a leak in the current mksurfdat code
       deallocate(pctlnd_pft_dyn)

    end if   ! end of if-create dynamic landust dataset   

    ! need to deallocate some memory here
    ...
    ! this tracks whether or not to open the diagnostic file
    first_call = .false.
